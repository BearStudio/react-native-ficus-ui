...
```js
import { Tabs, TabList, Tab, TabPanel, TabPanels } from "@ficus-ui/native";
```
...
### `colorScheme`
<PropsTable
  description="The colorScheme property, will define the tabs' main color."
...
### `onChangeTab`
<PropsTable
  description="Callback called when the current selected tab has changed."
